...
const fs = require('fs')
const tencentServerlessNodejs = require('../index')
...
const lambdaFunction = {
  handler: (event, context, resolutionMode, callback, _server) => tencentServerlessNodejs.proxy(_server || server, event, context, resolutionMode, callback)
}
...
describe('integration tests', () => {
  ...
  test('GET JSON single (resolutionMode = PROMISE; new server)', (done) => {
    ...
    const newServer = tencentServerlessNodejs.createServer(app)
    lambdaFunction.handler(makeEvent({
      path: '/users/1',
      httpMethod: 'GET'
  ...
  test('success - image response', (done) => {
    ...
    const serverWithBinaryTypes = tencentServerlessNodejs.createServer(app, null, ['image/*'])
    tencentServerlessNodejs.proxy(serverWithBinaryTypes, makeEvent({
      path: '/sam',
      httpMethod: 'GET'
    }), {
      succeed
  ...
  test('forwardLibraryErrorResponseToApiGateway', (done) => {
    ...
    tencentServerlessNodejs.proxy(server, null, {
      succeed
    })
  })
  ...
  test('serverListenCallback', (done) => {
    ...
    const serverWithCallback = tencentServerlessNodejs.createServer(mockApp, serverListenCallback)
    const succeed = response => {
    ...
    tencentServerlessNodejs.proxy(serverWithCallback, makeEvent({}), {
      succeed
    })
  })

  test('server.onError EADDRINUSE', (done) => {
    const serverWithSameSocketPath = tencentServerlessNodejs.createServer(mockApp)
    ...
    tencentServerlessNodejs.proxy(serverWithSameSocketPath, makeEvent({}), {
      succeed
    })
  })
